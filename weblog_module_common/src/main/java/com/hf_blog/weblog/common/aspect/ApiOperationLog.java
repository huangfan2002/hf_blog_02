...
//RetentionPolicy.RUNTIME 表示该注解将在运行时保留，
//这意味着它可以通过反射在运行时被访问和解析。

//@Target({ElementType.METHOD})：
//这个元注解用于指定注解的目标元素，即可以在哪些地方使用这个注解。
//ElementType.METHOD 表示该注解只能用于方法上。
//这意味着您只能在方法上使用这个特定的注解。
...
//aspectj 注解说明 在配置 AOP 切面之前，我们需要了解下 aspectj 相关注解的作用：
// @Aspect：声明该类为一个切面类；
//@Pointcut：定义一个切点，后面跟随一个表达式，表达式可以定义为切某个注解，
// 也可以切某个 package 下的方法；

//切点定义好后，就是围绕这个切点做文章了：
//@Before: 在切点之前，织入相关代码；
//@After: 在切点之后，织入相关代码;
//@AfterReturning: 在切点返回内容后，织入相关代码，
// 一般用于对返回值做些加工处理的场景；
